...
const DynamicContentEditor = ({ value = "", onChange }) => {
  ...
  const updateContent = (updatedBlocks) => {
    ...
    onChange(
      updatedBlocks
        .map(
          (b) =>
            `<block type=\"${b.type}\" color=\"${
              b.color
            }\"><![CDATA[${encodeURIComponent(b.content)}]]></block>`
        )
        .join("\n")
    );
  };
  ...
  // ADD A CODE BLOCK AT ANY POSITION
    ...
    <div className="w-full bg-gray-50 dark:bg-gray-900 rounded-lg shadow-md  sm:p-1 ">
      {blocks.map((block, index) => (
        ...
        >
          ...
          <button
  className="group relative flex items-center justify-center w-full px-4 py-2 rounded-lg border border-gray-300 bg-gray-100 text-gray-700 
           transition-all hover:bg-gray-200 hover:shadow-md focus:ring-2 focus:ring-blue-400 focus:outline-none 
           dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white"
  onClick={() => {
    const confirmAdd = window.confirm("Are you sure you want to add a block above?");
    if (confirmAdd) {
      addBlockAbove(index);
    }
  }}
>
  <span className="transition-transform group-hover:-translate-y-0.5">
    <AiOutlinePlus className="w-5 h-5 transition-transform group-hover:scale-110" />
  </span>
  <span className="ml-2 font-medium">Add Block Above</span>
</button>
            ...
            {block.type !== "image" && (
              <div className="flex flex-col sm:flex-row gap-2">
                <select
                  ...
                  value={block.color
                    .split(" ")
                    .find((c) => !c.startsWith("dark:"))}
                  onChange={(e) =>
                    handleNormalColorChange(index, e.target.value)
                  }
                >
            ...
            <button
              ...
              onClick={() => {
                const userInput = prompt(
                  "Type 'deleteThis' to confirm deletion:"
                );
                if (userInput === "deleteThis") {
                  removeBlock(index);
                } else {
                  alert(
                    "Deletion canceled. Please type 'deleteThis' correctly to confirm."
                  );
                }
              }}
            >
